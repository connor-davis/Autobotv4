...
// ReSharper disable all inspections
    ...
    public class api
    {
        ...
        public void web_login()
        {
            ...
            if (request.HttpMethod == "OPTIONS")
            {
                ...
                Thread.Sleep(
                    1); // without this, the response doesn't return to the website, and the web buttons can't be shown
                listener.Stop();
                goto start;
            }
        ...
        {
            string folder = @"Logs", file = Path.Combine(folder, "AutobotErrorLogs.txt");
        ...
        private static string req(NameValueCollection post_data)
        {
            try
            {
                using (WebClient client = new WebClient())
                {
                    ...
                    sigCheck(Encoding.Default.GetString(raw_response), client.ResponseHeaders["signature"],
                        post_data.Get(0));
        ...
        private static bool assertSSL(object sender, X509Certificate certificate, X509Chain chain,
            SslPolicyErrors sslPolicyErrors)
        {
            if ((!certificate.Issuer.Contains("Cloudflare Inc") &&
                 !certificate.Issuer.Contains("Google Trust Services") &&
                 !certificate.Issuer.Contains("Let's Encrypt")) || sslPolicyErrors != SslPolicyErrors.None)
            {
                error(
                    "SSL assertion fail, make sure you're not debugging Network. Disable internet firewall on router if possible. & echo: & echo If not, ask the developer of the program to use custom domains to fix this.");
                return false;
            }
        ...
        private static void sigCheck(string resp, string signature, string type)
        {
            ...
            try
            {
                ...
                {
                    error(
                        "Signature checksum failed. Request was tampered with or session ended most likely. & echo: & echo Response: " +
                        resp);
                    Environment.Exit(0);
                }
            }
        ...
        private void load_user_data(user_data_structure data)
        {
            ...
            user_data.subscriptions =
                data.subscriptions; // array of subscriptions (basically multiple user ranks for user with individual expiry dates 
        }
